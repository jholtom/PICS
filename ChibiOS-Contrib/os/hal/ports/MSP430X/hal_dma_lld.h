 ...
 */
typedef struct {
  const volatile void * source_addr;  /**< @brief Source address                       */
  volatile void * dest_addr;          /**< @brief Destination address                  */
...
/** @brief    Determines whether a DMA channel has been claimed.
 * 
 * @param[in] pch     Pointer to the channel to check
 */
#define dmaIsClaimed(pch) ( ((pch) != NULL) && \
    ((pch)->registers != NULL) && \
    (((pch)->registers->ctl & DMAEN) != 0) )
