...
def CmdUploadFW(addr, chunk, crc = False, reply = False, address = None):
    ...
    r += struct.pack('!I%dB' % len(chunk), addr, *payload)
    r += footer(r, crc)
    return r
...
def CmdStoreTelem(chunk, crc = False, reply = False, address = None):
    ...
    r += struct.pack('!%dB' % len(chunk), addr, *payload)
    r += footer(r, crc)
    return r

def CmdGetTelem(duration, idx_start = None, idx_end = None, ts_start = None, ts_end = None, crc = False, reply = False, address = None):
    ...
    if idx_start and idx_end and ts_start and ts_end:
        r += struct.pack('!HBBii', duration, idx_start, idx_end, ts_start, ts_end)
    elif ts_start and ts_end:
        r += struct.pack('!Hii', duration, ts_start, ts_end)
    elif idx_start and idx_end:
        r += struct.pack('!HBB', duration, idx_start, idx_end)
    else:
        r += struct.pack('!H', duration)
